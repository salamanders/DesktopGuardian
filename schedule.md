...
To ensure the task runs even if the computer was off at the scheduled time, we use an XML definition with `<StartWhenAvailable>true</StartWhenAvailable>`.

**Command to Create Task:**

```shell
schtasks /Create /XML task_definition.xml /TN "DesktopGuardianScan" /F
```

* **`/XML ...`**: Import task definition from an XML file (generated by the app).
